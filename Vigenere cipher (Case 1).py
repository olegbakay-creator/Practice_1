def vigenere_encrypt(text, key):
    ...
    # Предобработка: верхний регистр, оставляем только буквы алфавита
    text = text.upper()
    clean_text = "".join([char for char in text if char in alphabet])
    key = key.upper()
    ...
    for i, char in enumerate(clean_text):
        ...
        k_idx = alphabet.index(key[i % key_length])
        
        # Применяем формулу шифра Вижинера
        c_idx = (p_idx + k_idx) % 32
        encrypted_text += alphabet[c_idx]
        
    return encrypted_text

# Ваш исходный текст
source_text = """История развития систем информационной безопасности насчитывает множество этапов, каждый из которых характеризуется своими особенностями и достижениями. Первые попытки защиты информации связаны с появлением шифрования, которое возникло еще до нашей эры. В Древнем Египте использовались специальные знаки для кодировки сообщений, а в Римской империи применяли различные методы шифровки для военных целей.
...
# Шифрование
key = "КРИПТОГРАФИЯ"
result = vigenere_encrypt(source_text, key)
print(result)